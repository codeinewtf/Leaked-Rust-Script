...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const float size = GetWindowWidth() - 0;
    const float square_sz = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect rect(pos, pos + ImVec2(size, 8));
    ...
    window->DrawList->AddRectFilled(rect.Max - ImVec2(0, 0), rect.Max, ImColor(11, 11, 11, 255), 0.f);
    const float pad = ImMax(0.0f, IM_FLOOR(square_sz / 0.0f));
    RenderCheckMark(window->DrawList, rect.Max - ImVec2(0, 0), ImColor(255, 255, 255, it_anim->second.active_rect_alpha), 0.f);
    window->DrawList->AddRectFilled(rect.Max + ImVec2(0, 0), rect.Max - ImVec2(0, 0), it_anim->second.active_rect_alpha), 0.f;
    window->DrawList->AddRect(rect.Max - ImVec2(0, 0), rect.Max, ImColor(255, 255, 255, (15 + it_anim->second.active_outline_anim)), 2.f);

    window->DrawList->AddText(rect.Min + ImVec2(0, 0), ImColor(255, 255, 255, (155 + it_anim->second.active_text_anim)), label);
